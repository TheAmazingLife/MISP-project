...
// tipo de cromosoma: vector de pares (valor double usado para orden/ranking, id entero)
...
public:
  ...
  */
  ...
  BRKGA(int n, int p, double pe, double pm, double rhoe, double s, std::vector<std::vector<int>> &adj);
  ~BRKGA();
  ...
  //ejecuta la generacion: crea una nueva poblacion aplicando elite, cruzamiento y mutacion
