#!/bin/bash
#SBATCH --job-name=battle_brkga
#SBATCH --output=resultados/brkga_%x_%j.stdout
#SBATCH --error=resultados/brkga_%x_%j.stderr
...
# Battle Royale: Ejecución de BRKGA con seguimiento Anytime
...
if [ -z "$INSTANCE" ]; then
    ...
    echo "Uso: sbatch run_brkga_anytime.sh <ruta_instancia> [tiempo_segundos] [intervalo_muestreo]"
    echo "Ejemplo: sbatch run_brkga_anytime.sh dataset_grafos_no_dirigidos/new_3000_dataset/erdos_n3000_p0c0.7_1.graph 3600 5"
    exit 1
fi

# Extraer nombre base del grafo para el output
BASENAME=$(basename "$INSTANCE" .graph)
OUTPUT_FILE="${BASE_DIR}/battleroyale/resultados/brkga_${BASENAME}_${TIME}s.csv"

# Información del job
echo "═══════════════════════════════════════════════════════════"
echo "  BATTLE ROYALE - BRKGA Anytime"
echo "═══════════════════════════════════════════════════════════"
...
# Ejecutar BRKGA con seguimiento anytime
